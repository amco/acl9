      ...
      def acts_as_authorization_object(options = {})
        ...
        has_many :"#{subj_table}",
          ->{
              ...
              where(authorizable_id: id).
              joins("INNER JOIN ? ON ?_id = ?.id", join_table, subj_col, subj_table).
              joins("INNER JOIN ? ON ?.id = ?_id", role_table, role_table, role.underscore).
              select("SELECT DISTINCT ?.*", subj_table)
            }
